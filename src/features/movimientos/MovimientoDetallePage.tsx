...
export function MovimientoDetallePage() {
  ...
  return (
    <section className="space-y-5">
      ...
      {cargando && <Notice>Cargando movimiento...</Notice>}{error && <Notice tone="danger">{error}</Notice>}
      {movimiento && <>
        <Panel className="space-y-4">
          ...
          {movimiento.detallesReposicion.map((detalle) => <div key={detalle.id} className="flex justify-between gap-3 rounded-2xl bg-black/15 p-3 text-sm"><span>{detalle.cantidad} × {detalle.producto?.nombre ?? "Producto eliminado"}</span><span className="font-semibold">{formatearPesos(detalle.subtotal)}</span></div>)}
