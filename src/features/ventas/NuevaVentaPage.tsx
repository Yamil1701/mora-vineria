...
export function NuevaVentaPage() {
  ...
  async function guardarVenta() {
    ...
    if (!await confirm({ title:"Confirmar venta", description:<div className="space-y-2"><ul className="space-y-1 text-white/80">{carrito.slice(0, 3).map((item) => <li key={item.productoId}>{productosPorId.get(item.productoId)?.nombre ?? "Producto"} · {item.cantidad}</li>)}</ul>{carrito.length > 3 && <p className="text-sm text-white/55">Y {carrito.length - 3} producto{carrito.length - 3 === 1 ? "" : "s"} más.</p>}<p className="border-t border-white/10 pt-2 font-semibold">Total: {formatearPesos(total)}</p></div>, confirmLabel:"Guardar venta" })) return;
  ...
  return <section className="space-y-5 pb-24">
    ...
    <Panel className="space-y-3">
      ...
      {!cargando && productosFiltrados.length===0 && <div className="py-6 text-center text-sm text-white/50"><p>{carrito.length && !busqueda ? "Todos los productos disponibles ya están en el carrito." : productos.length ? "No encontramos productos disponibles." : "Primero necesitás cargar un producto."}</p>{!productos.length && !esConsulta && <Link to="/productos/nuevo" className="mt-3 inline-flex min-h-12 items-center font-semibold text-mora-suave">Agregar primer producto</Link>}</div>}
    </Panel>
