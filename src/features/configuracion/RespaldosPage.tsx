...
import { Button, ErrorState, Notice, Panel, TaskHeader, useConfirm } from "../../components/ui";
...
export function RespaldosPage() {
  ...
  const { ultimoRespaldo } = useEstadoRespaldo();
  ...
  return <section className="space-y-5">
    ...
    <Panel className="space-y-4"><div><h2 className="text-lg font-semibold">Crear una copia</h2><p className="mt-1 text-sm leading-6 text-white/60">Guardala fuera de este dispositivo para poder recuperar los datos.</p>{ultimoRespaldo && <p className="mt-2 text-xs text-white/45">Último respaldo: {formatearFecha(ultimoRespaldo.exportedAt)}</p>}</div><Button size="lg" fullWidth disabled={procesando} onClick={() => void exportarBackup("descargar")}>Descargar respaldo JSON</Button><Button variant="secondary" fullWidth disabled={procesando} onClick={() => void exportarBackup("compartir")}>Compartir respaldo</Button></Panel>
