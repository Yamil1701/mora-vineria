...
export function RouteSheet({ label, children }: { label: string; children: ReactNode }) {
  ...
  return <UnsavedChangesContext.Provider value={setHasUnsavedChanges}><Dialog.Root open={open} onOpenChange={(nextOpen) => { if (!nextOpen) void closeWithAnimation(); }}><Dialog.Portal>
