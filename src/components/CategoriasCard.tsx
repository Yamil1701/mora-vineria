...
export function CategoriasCard({ onCategoriasChange, soloConsulta = false }: { onCategoriasChange?: () => void; soloConsulta?: boolean }) {
  ...
  return <section className="space-y-4">
    ...
    <BottomSheet open={abierta} preventClose={confirmarSalida} onOpenChange={(open)=>{if(!open){setDirty(false);setSeleccionada(null);setModo("detalle");}}} title={modo==="crear"?"Nueva categoría":modo==="editar"?"Editar categoría":seleccionada?.nombre??"Categoría"}>
      {modo==="crear"||modo==="editar"?<form onSubmit={(e)=>void guardar(e)} className="space-y-4"><label><span className="text-sm text-white/70">Nombre</span><Input autoFocus value={nombre} onChange={e=>{setNombre(e.target.value);setDirty(true);setErrorNombre(null);}} placeholder="Ej: Vermut" />{errorNombre&&<span role="alert" className="mt-1 block text-xs text-red-200">{errorNombre}</span>}</label><div className="grid grid-cols-2 gap-3"><Button variant="secondary" onClick={async()=>{if(await confirmarSalida()){setDirty(false);if(modo==="editar")setModo("detalle");else setSeleccionada(null);}}}>Cancelar</Button><Button type="submit" disabled={guardando}>{guardando?"Guardando...":"Guardar"}</Button></div></form>:seleccionada&&<div className="space-y-4"><div className="rounded-2xl bg-white/[.05] p-4"><p className="text-sm text-white/50">Productos asociados</p><p className="mt-1 text-2xl font-bold">{productos.filter(p=>p.categoriaId===seleccionada.id).length}</p><p className="mt-2 text-sm text-white/55">Estado: {seleccionada.activa?"Activa":"Inactiva"}</p></div>{!soloConsulta&&<div className="grid gap-3"><Button onClick={()=>{setDirty(false);setModo("editar");}}>Editar categoría</Button><Button variant={seleccionada.activa?"warning":"success"} onClick={()=>void cambiarEstado()}>{seleccionada.activa?"Desactivar":"Activar"}</Button><Button variant="danger" onClick={()=>void eliminar()}>Eliminar categoría</Button></div>}</div>}
    </BottomSheet>
  </section>;
}
