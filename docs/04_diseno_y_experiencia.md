...
La búsqueda concentra la selección y oculta productos ya agregados. El carrito vive en un bottom sheet accesible desde una barra inferior; permite quitar explícitamente, cambiar cantidades, editar precio y vaciar. La revisión y el cobro continúan dentro del sheet. El aviso de recuperación aparece solo cuando existían productos antes de abrir la tarea. Efectivo ofrece cálculo transitorio de vuelto y Transferencia exige elegir destino. En pago combinado se escribe el primer importe y el segundo completa el total; ambos medios y cuentas quedan visibles antes de confirmar. Al guardar se vuelve al historial: la venta destaca una sola vez, su fondo se desvanece y los productos tienen más jerarquía que la fecha.
